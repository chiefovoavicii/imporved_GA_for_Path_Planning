...
class ObstacleManager:
    ...
    def initialize_random_obstacles(self):
        """随机障碍物生成"""
        num_obstacles = int(self.obstacle_density * self.grid.grid_size ** 2)
        indices = np.random.choice(self.grid.grid_size ** 2, num_obstacles, replace=False)
        for idx in indices:
            x, y = idx // self.grid.grid_size, idx % self.grid.grid_size
            self.obstacles[x, y] = 1
